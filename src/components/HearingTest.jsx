...
export default function HearingTest({ userData, onComplete }) {
  ...
  const playCalibrationTone = () => {
    setIsPlaying(true)
    playTone(1000, 2000, 'left')
    setTimeout(() => {
      playTone(1000, 2000, 'right')
    }, 2500)
    setTimeout(() => {
      setIsPlaying(false)
    }, 5000)
          ...
          <div className="glass p-6 sm:p-8">
            <div className="text-center mb-8">
              ...
              <p className="text-clinical-600 mb-6">
                Calibrate audio output levels for standardized testing conditions.
              </p>
            </div>

            <div className="bg-clinical-50 rounded-md p-6 mb-6 border border-clinical-200">
              <h3 className="font-bold text-clinical-900 mb-3">Calibration Instructions:</h3>
              <ul className="space-y-2 text-clinical-700 text-sm">
                <li>• Activate test tone via button below</li>
                <li>• Binaural presentation: left ear, then right ear</li>
                <li>• Tone should be audible but comfortable</li>
                <li>• Adjust device volume as necessary</li>
              </ul>
            </div>
            ...
            <div className="flex justify-center mb-6">
              <button
                ...
                disabled={isPlaying}
                className={`w-32 h-32 rounded-md flex flex-col items-center justify-center transition-all ${
                  isPlaying
                    ? 'bg-primary-400 cursor-not-allowed'
                    : 'bg-primary-600 hover:bg-primary-700'
                }`}
              >
                <Volume2 className={`w-12 h-12 text-white mb-2 ${isPulsing ? 'animate-pulse' : ''}`} />
                <span className="text-white text-sm font-semibold">
                  {isPlaying ? 'Playing...' : 'Test Tone'}
                </span>
              </button>
            </div>
